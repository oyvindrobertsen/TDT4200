...
__global__ void convertImageToNewFormatGPU(PPMPixel* imageData, AccuratePixel* imageAccurateData) {
    ...
    imageAccurateData[i].red = (float) imageData[i].red;
    imageAccurateData[i].green = (float) imageData[i].green;
    imageAccurateData[i].blue = (float) imageData[i].blue;
}
...
AccurateImage *convertImageToNewFormat(PPMImage *image) {
    ...
    convertImageToNewFormatGPU<<<threadsPerBlock, blocks>>>(devImageData, devAccurateImageData);
...
void performNewIdeaIteration(AccurateImage *imageOut, AccurateImage *imageIn, int size) {
    ...
    AccuratePixel* devImageInData;
    AccuratePixel* devImageOutData;
    int* devSize;
    ...
    performNewIdeaIterationGPU<<<threadsPerBlock, blocks>>>(devImageInData, devImageOutData, devSize);
...
__device__ unsigned char threshold(float value) {
    if(value > 255.0f)
        return 255;
    else if (value < -1.0f) {
        value = 257.0f+value;
...
__global__ void performNewIdeaFinalizationGPU(
        AccuratePixel* imageInSmallData, AccuratePixel* imageInLargeData, PPMPixel* imageOutData) {
    // Calculate data-index
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int w = gridDim.x * blockDim.x;
    ...
    float value = (imageInLargeData[i].red - imageInSmallData[i].red);
    imageOutData[i].red = threshold(value);

    value = (imageInLargeData[i].green - imageInSmallData[i].green);
    imageOutData[i].green = threshold(value);

    value = (imageInLargeData[i].blue - imageInSmallData[i].blue);
...
void performNewIdeaFinalization(AccurateImage *imageInSmall, AccurateImage *imageInLarge, PPMImage *imageOut) {
    ...
    AccuratePixel* devImageInSmallData;
    AccuratePixel* devImageInLargeData;
    ...
    performNewIdeaFinalizationGPU<<<threadsPerBlock, blocks>>>(devImageInSmallData, devImageInLargeData, devImageOutData);

    // Retrieve result
    cudaMemcpy(imageOut->data, devImageOutData, sizeof(PPMPixel) * imageOut->x * imageOut->y, cudaMemcpyDeviceToHost);
